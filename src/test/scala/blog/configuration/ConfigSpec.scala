  ...
  test("load chromedriver configurations correctly") {
    ...
    for {
      ...
      expect(config.chromedriver.options == List("--remote-allow-origins=*", "--disable-gpu", "--window-size=1920,1080", "--no-sandbox", "--disable-dev-shm-usage")) &&
        expect(config.chromedriver.path == PathConfig("webdriver.chrome.driver", "/usr/local/Caskroom/chromedriver/128.0.6613.86/chromedriver-mac-x64/chromedriver", "/home/runner/.nix-profile/bin/chromedriver", "/usr/bin/chromedriver"))
    }
  }
}
