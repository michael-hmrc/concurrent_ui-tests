...
object ConfigSpec extends SimpleIOSuite {
  ...
  test("load chromedriver configurations correctly") {
    ...
    for {
      ...
      expect(config.chromedriver.options == List("--remote-allow-origins=*", "--disable-gpu", "--no-sandbox", "--disable-dev-shm-usage")) &&
        expect(config.chromedriver.path == PathConfig("webdriver.chrome.driver", "/usr/local/Caskroom/chromedriver/126.0.6478.126/chromedriver-mac-x64/chromedriver", "/home/runner/.nix-profile/bin/chromedriver"))
    }
  }
}
